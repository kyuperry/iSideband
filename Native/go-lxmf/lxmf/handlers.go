...
func (h *DeliveryAnnounceHandler) ReceivedAnnounce(destinationHash []byte, announcedIdentity *rns.Identity, appData []byte) {
	...
	rns.Log(
		fmt.Sprintf(
			"DELIVERY HANDLER ENTERED destination=%s pending=%d",
			rns.PrettyHexRep(destinationHash),
			len(h.Router.PendingOutbound),
		),
		rns.LOG_NOTICE,
	)
	...
	for _, msg := range h.Router.PendingOutbound {
		...
		rns.Log(
			fmt.Sprintf("PATH RESPONSE: %s", rns.PrettyHexRep(destinationHash)),
			rns.LOG_NOTICE,
		)

		rns.Log(
			fmt.Sprintf("QUEUED MESSAGE: %s", rns.PrettyHexRep(msg.DestinationHash)),
			rns.LOG_NOTICE,
		)
